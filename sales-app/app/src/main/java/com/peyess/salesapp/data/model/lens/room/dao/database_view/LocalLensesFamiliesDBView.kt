...
private const val lensFamiliesTable = LocalLensFamilyEntity.tableName
private const val lensesDetailsTable = LocalLensDetailsCrossRef.tableName

@DatabaseView(
    viewName = LocalLensesFamilyDBView.viewName,
    value = """
        SELECT
            lensesFamilies.id AS id,
            lensesFamilies.name AS name,
         ...
         FROM $lensFamiliesTable AS lensesFamilies
         JOIN $lensesDetailsTable AS lensesDetails ON lensesFamilies.id = lensesDetails.brand_id
    """
)
data class LocalLensesFamilyDBView(
    val id: String = "",
    val name: String = "",
    ...
    companion object {
        const val viewName = "local_lenses_family_db_view"
    }
}
